...
<div class="menu">
  ...
  <div class="menu-button menu-button-active"  DATA_func="sr">
    Search
  </div>
...
<p><label for="content">Search:</label><br/>
<input type="text" id="content" name="content" size="30" /></p>
...
$result = mysql_query("SELECT * FROM papers
   WHERE paperID LIKE '%$tmp1%' || title LIKE '%$tmp1%' || field LIKE '%$tmp1%' ||
   	year LIKE '%$tmp1%' || bookmarknum LIKE '%$tmp1%' || periodical LIKE '%$tmp1%' ||
   	  submitor LIKE '%$tmp1%' || researchers LIKE '%$tmp1%'");
...
$rrrrrr=mysql_query("SELECT valid from bookmarks WHERE paperID='$tmp3' AND studentID='".$_SESSION['studentID']."'" );
  ...
  if(!$outans) {
    ...
    <input type=\"hidden\" name=\"content\" value= '".$tmp1."'/>
